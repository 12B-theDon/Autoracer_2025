...
public:
  MPCBicycleNode() : Node("mpc_bicycle_node")
  {
    ...
    start_topic_      = declare_parameter<std::string>("start_topic", "mpc_start_rotary");
    ...
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "/odom", 10, std::bind(&MPCBicycleNode::odomCb, this, _1));
    start_sub_ = this->create_subscription<std_msgs::msg::Bool>(
      start_topic_, 10, std::bind(&MPCBicycleNode::startTriggerCb, this, _1));
    ...
    RCLCPP_INFO(get_logger(), "MPCBicycleNode initialized. Waiting for start signal on %s.",
      start_topic_.c_str());
  }
...
private:
  ...
  void startTriggerCb(const std_msgs::msg::Bool::SharedPtr msg) {
    if (!msg->data) {
      return;
    }
    if (!start_signal_received_) {
      start_signal_received_ = true;
      RCLCPP_INFO(get_logger(), "Received start trigger on %s. Enabling MPC controller.",
        start_topic_.c_str());
    }
  }
  ...
  void controlLoop() {
    if (!start_signal_received_) {
      RCLCPP_INFO_THROTTLE(get_logger(), *this->get_clock(), 2000,
        "Waiting for start trigger on %s...", start_topic_.c_str());
      return;
    }
